...
static enum CHIP_PS_STATE_T genuChipPSstate = CHIP_WAKEDUP;
...
inline void acquire_bus(enum BUS_ACQUIRE acquire)
{
...
inline void release_bus(enum BUS_RELEASE release)
{
	#ifdef WILC_OPTIMIZE_SLEEP_INT
...
static int wilc_wlan_txq_filter_dup_tcp_ack(void)
{
	...
	while (Dropped > 0) {
		...
		if(down_timeout(p->txq_wait, msecs_to_jiffies(1)))
			PRINT_ER("semaphore timedout\n");
		Dropped--;
	}
...
INLINE void chip_wakeup(void)
{
	...
	do
	{
		...
		do
		{
			...
			{
				PRINT_ER("Couldn't read chip id. Wake up failed\n");
			}
		...
		#endif
			{
				if (!is_cfg_packet) {
					if (p->net_func.rx_indicate) {
						if (pkt_len > 0) {
							p->net_func.rx_indicate(&buffer[offset], pkt_len, pkt_offset, PKT_STATUS_NEW);
							has_packet = 1;
						}
					}
				} else {
...
static int wilc_wlan_start(void)
{
	...
	if (!ret) {
		PRINT_ER("[wilc start]: fail read reg 0x1000 ...\n");
		release_bus(RELEASE_ONLY);
		ret = -5;
 		return ret;
	}
...
uint32_t wilc_get_chipid(uint8_t update)
{
	...
	if(chipid == 0 || update != 0) {
		ret = g_wlan.hif_func.hif_read_reg(0x1000,&tempchipid);
		if (!ret)
			PRINT_ER( "[wilc start]: fail read reg 0x1000 ...\n");
		ret = g_wlan.hif_func.hif_read_reg(0x13f4,&rfrevid);
		if (!ret)
			PRINT_ER( "[wilc start]: fail read reg 0x13f4 ...\n");
